...
inline void read_tensor_file(char* file) {
  ...
  fread(&x,sizeof(int),1,ten);fread(&y,sizeof(int),1,ten);fread(&z,sizeof(int),1,ten);
  fread(&x_min,sizeof(float),1,ten);fread(&y_min,sizeof(float),1,ten);fread(&z_min,sizeof(float),1,ten);
  fread(&res,sizeof(float),1,ten);
  nb_elementsX = x; nb_elementsY = y; nb_elementsZ = z;
  ...
  for(k=0; k<z; k++)
    for(j=0; j<y; j++)
      for(i=0; i<x; i++){
	//        fscanf(ten,"%e",&gridXX[i][j][k]);
	fread(&gridXX[i][j][k],sizeof(float),1,ten);
	if(isnan(gridXX[i][j][k]))printf("%i %i %i %s\n",i,j,k,"XX NaN" );}
  for(k=0; k<z; k++)
    for(j=0; j<y; j++)
      for(i=0; i<x; i++){
	//        fscanf(ten,"%e",&gridXY[i][j][k]);
	fread(&gridXY[i][j][k],sizeof(float),1,ten);
	if(isnan(gridXY[i][j][k]))printf("%i %i %i %s\n",i,j,k,"XY NaN" );}
  for(k=0; k<z; k++)
    for(j=0; j<y; j++)
      for(i=0; i<x; i++){
	//        fscanf(ten,"%e",&gridYY[i][j][k]);
	fread(&gridYY[i][j][k],sizeof(float),1,ten);
	if(isnan(gridYY[i][j][k]))printf("%i %i %i %s\n",i,j,k,"YY NaN" );}
  for(k=0; k<z; k++)
    for(j=0; j<y; j++)
      for(i=0; i<x; i++){
	//        fscanf(ten,"%e",&gridXZ[i][j][k]);
	fread(&gridXZ[i][j][k],sizeof(float),1,ten);
	if(isnan(gridXZ[i][j][k]))printf("%i %i %i %s\n",i,j,k,"XZ NaN" );}
  for(k=0; k<z; k++)
    for(j=0; j<y; j++)
      for(i=0; i<x; i++){
	//        fscanf(ten,"%e",&gridYZ[i][j][k]);
	fread(&gridYZ[i][j][k],sizeof(float),1,ten);
	if(isnan(gridYZ[i][j][k]))printf("%i %i %i %s\n",i,j,k,"YZ NaN" );}
  for(k=0; k<z; k++)
    for(j=0; j<y; j++)
      for(i=0; i<x; i++){
	//        fscanf(ten,"%e",&gridZZ[i][j][k]);
	fread(&gridZZ[i][j][k],sizeof(float),1,ten);
	if(isnan(gridZZ[i][j][k]))printf("%i %i %i %s\n",i,j,k,"ZZ NaN" );}
